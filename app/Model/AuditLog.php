...
class AuditLog extends AppModel
{
    ...
    public function beforeSave($options = array())
    {
        ...
        $this->__writeAuditMetrics($this->data);
    ...
    /**
     * Write Audit Log to metrics
     *
     * @return void
     */
    private function __writeAuditMetrics(array $data)
    {
        if (!Configure::read('Metrics.enabled')) {
            return;
        }

        $this->getMetricsTool()->writeAuditLog(
            $data['AuditLog']['model'],
            $data['AuditLog']['action'],
            $data['AuditLog']['ip'],
            $data['AuditLog']['model_id'],
            $data['AuditLog']['event_id']
        );
    }
}
